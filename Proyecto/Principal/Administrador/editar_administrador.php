...
<form id="formulario" action="administradores_salva_actualiza.php" method="POST" enctype="multipart/form-data"> <!-- Agregar enctype="multipart/form-data" para permitir la carga de archivos -->
    ...
    <input type="email" name="correo" id="correo" value="<?php echo $row['correo']; ?>" onblur="validarCorreo();">
    ...
    <select name="rol" id="rol">
      ...
      <option value="0" <?php if ($row['rol'] == 0) echo "selected"; ?>>Ejecutivo</option>
    </select>
    <br><br>
...
$(document).ready(function() {
  $('form').submit(function(event) {
    ...
    if (error) {
      ...
      // Realizar la petición AJAX solo si se ha modificado el correo
      if (correo !== '<?php echo $row['correo']; ?>') {
        $.ajax({
        ...
        // Si no se ha modificado el correo, enviar el formulario
        $('form').unbind('submit').submit();
      }
    }
  });
